...
def get_k_coefficients(t: float, y: np.ndarray, h: float, f: Callable[[Any, Any], np.ndarray], tableau: dict) -> List[np.ndarray]:
    a_ = tableau['a_']
    c_ = tableau['c_']
    k = []
...
def rk_one_step(t: float, y: np.ndarray, h: float, f: Callable[[Any, Any], np.ndarray], tableau: dict) -> Tuple[float, np.ndarray]:
...
def rk(t0: float, t_end: float, y0: np.ndarray, h: float, f: Callable[[Any, Any], np.ndarray], tableau: dict) -> Tuple[np.ndarray, np.ndarray]:
...
def error_function(K: np.ndarray, t_obs: np.ndarray, y_obs: np.ndarray, y0: np.ndarray, h: float, L_eq: List[float], tableau: dict) -> float:
...
if __name__ == "__main__":
    ...
    y_restored = rk(0, t_end, y0, h, lambda t, y: compute_forces(t, y, K_estimated, L_eq), tableau)[1]

    # Расчет разности между истинными и восстановленными данными
    diff = y_true - y_restored

    # Построение графика разности
    plt.figure()
    for i in range(0, y_true.shape[0], 2):  # Графики для x-координат каждой массы
        plt.plot(t, diff[i], label=f'Разность для массы {i // 4 + 1} (x)')
    plt.xlabel('Время')
    plt.ylabel('Разность')
    plt.title('Разность между истинными и восстановленными данными')
    plt.legend()
    plt.grid()
